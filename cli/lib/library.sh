...
boot_service() {
  ...
  if [ "$CONTROL_PLANE_STATUS" != "FAILED" ]; then
    PAYLOAD="{\"name\":\"sp-$1\",\"imageId\":\"sp_platform/spi_$1\",\"env\": {\"DNSHOST\": \"$1.$2\", \"MUON_DOMAIN\": \"$2\",\"MUON_CONTROL_PLANE_PORT\": \"8080\", \"MUON_CONTROL_PLANE_IP\": \"$CONTROL_PLANE\"}, \"dependencies\":[{\"dependency\":\"sp-control-plane\",\"host\":\"$CONTROL_PLANE\",\"port\":8080}]}"
